...
class Development extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Milestone_development_model');
		$this->load->model('Detail_development_model');
	}

	public function index()
	{
        $data['tittle'] = "Development";
        $data['isi'] 	= "v_milestone/development/development";
		$data['data']	= base_url('c_milestone/development/data');
		$data['detail']	= base_url('c_milestone/development/detail');
		$data['show']	= base_url('c_milestone/development/show_detail');
	...
	{	
		...
		$id = $this->input->get('development',true);
		$where['id'] = $id;
		$tl = $this->Code_tl_model->get($where);
		if($tl){
			$data['tittle']	= 'Milestone Departemen '.$tl->departemen.' Team '.$tl->code_tl.' '.$tl->name;
			$data['isi']	= "v_milestone/development/add_development";
			$data['team']	= $tl;
			$data['table']	= base_url('c_milestone/development/table/');
			$data['tahun']	= base_url('c_milestone/development/tahun');
			$data['save']	= base_url('c_milestone/development/save');
			$data['edit']	= base_url('c_milestone/development/edit');
			$data['delete']	= base_url('c_milestone/development/delete');
			$this->load->view('layouts/wrapper',$data);
		} else {
			redirect(base_url('c_milestone/development'));
		}
	}

	public function table($id,$tahun)
	{
		$data['data'] = base_url('c_milestone/development/table_add/'.$id.'/'.$tahun);
		$this->load->view('v_milestone/development/table_development',$data);
	}
	...
	public function table_add($id,$tahun)
	{
        ...
        $no = $this->input->post('start');
        $list = $this->Milestone_development_model->lists(
            '   
		...
		$data['draw'] = $this->input->post('draw');
		$data['recordsTotal'] = $this->Milestone_development_model->list_count($where, true);
		$data['recordsFiltered'] = $this->Milestone_development_model->list_count($where, true);
        $data['data'] = $temp_data;
        echo json_encode($data);
	...
	public function save()
	{
		...
		if($id) { 
			// edit
			$this->Milestone_development_model->update($savedata, array('id' => $id));
			$message = array(
				'type' => 'success',
			...
			//create
			$this->Milestone_development_model->insert($savedata);
			$message = array(
				'type' => 'success',
	...
	public function edit()
	{
		...
		$data['data'] = $this->Milestone_development_model->get($where);
		echo json_encode($data);
	}
	...
	public function delete()
	{
		...
		$cek = $this->db->get_where('detail_development',['milestone_id' => $where['id']])->row_array();
		if($cek){
			$msg = array(
				'type' 	=> 'error',
			...
			$this->db->trans_begin();
			$this->Milestone_development_model->delete($where);
	...
	public function show_detail()
	{
		...
		$data['isi']	= "v_milestone/development/development_show";
		$data['tahun']	= base_url('c_milestone/development/tahun');
		$data['table']	= base_url('c_milestone/development/table_show/');
		$this->load->view('layouts/wrapper',$data);
	}

	public function table_show($id,$tahun)
	{
		$data['data'] = base_url('c_milestone/development/data_show/'.$id.'/'.$tahun);
		$this->load->view('v_milestone/development/table_development_show',$data);
	}

	public function data_show($id,$tahun)
	{
		$temp_data = [];
		$where['milestone_development.code_tl_id'] = $id;
		$where['milestone_development.tahun_milestone'] = $tahun;
        $no = $this->input->post('start');
        $list = $this->Milestone_development_model->lists(
            '   
				milestone_development.*,
	...
	public function check_detail()
	{
		...
		$cek = $this->db->get_where('detail_development',['milestone_id' => $id])->row_object();
		if($cek){
			$this->db->select('
							dv.id as id,
							dv.week as week,
						...
						');
			$this->db->from('detail_development dv');
	...
	public function all_detail_sales()
	{
		...
		$data['data'] = $this->Detail_development_model->get_all($where);
	...
	public function cek_milestone_id()
	{
		...
		if($id_detail){
			$milestone['data_detail'] = $this->db->get_where('detail_development',['id' => $id_detail])->row_array();
		}
		$milestone['data'] = $this->db->get_where('milestone_development',['id' => $id])->row_array();
	...
	public function save_detail()
	{
		...
		if($id) { 
			// edit
			$this->Detail_development_model->update($savedata, array('id' => $id));
			$message = array(
				'type' => 'success',
				'msg' => "Detail Milestone Development Berhasil di ubah!",
			);

		} else { 
			//create
			$milestone = $this->db->get_where('detail_development',['milestone_id' => $savedata['milestone_id'],'week' => $savedata['week']])->row_array();
			if($milestone !== null ){
				$message = array(
					'type' => 'error',
					'msg' => "Data sudah ada!"
				...
				$this->Detail_development_model->insert($savedata);
				$message = array(
					'type' => 'success',
					'msg' => "Detail Milestone Development Berhasil di tambah!",
				);
			}
		}
