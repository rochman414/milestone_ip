...
class Detail_sales_marketing_model extends CI_Model {

    public $table = 'detail_sales_marketing';
    public $primary_key = 'id';
    public $order_by = 'week';
    public $order_type = 'ASC';
    public $search_field = 'week';
    public $column_order = ['keterangan_update','week','kendala_pencapaian_milestone']; //set column field database for datatable orderable
    public $column_search = ['keterangan_update','week','kendala_pencapaian_milestone']; //set column field database for datatable searchable 
    ...
    function lists($select = '*', $where = null, $limit = 10 ,$offset = 0)
    {
        $this->db->select($select)
                 ->join('status_code','status_code.id = detail_sales_marketing.status_id','right')
                 ->limit($limit,$offset);
    ...
    function list_count($where = null, $is_where = false) {
        $this->db->join('status_code','status_code.id = detail_sales_marketing.status_id','right');      
        if($is_where) {
            if($where) {
                if(isset($where['q']) && $where['q'])
                    $this->db->like("LOWER(".$this->search_field.")", $where['q']);
