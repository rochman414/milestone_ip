...
class Detail_development_model extends CI_Model {

    public $table = 'detail_development';
    ...
    function lists($select = '*', $where = null, $limit = 10 ,$offset = 0)
    {
        $this->db->select($select)
                 ->join('status_code','status_code.id = detail_development.status_id','right')
                 ->limit($limit,$offset);
    ...
    function list_count($where = null, $is_where = false) {
        $this->db->join('status_code','status_code.id = detail_development.status_id','right');      
        if($is_where) {
            if($where) {
                if(isset($where['q']) && $where['q'])
                    $this->db->like("LOWER(".$this->search_field.")", $where['q']);
